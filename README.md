...
| **대상 예측** | 173종 → 너프 5 · 버프 5. **무작위 선택의 6.2배** |
| **방향 예측** | 너프인가 버프인가 — **AUC 0.900 · F1 0.809 · MCC 0.617** |
| **위험 경고** | 프로 경기 단골을 버프하면 대회 출전 **+37%**, 그 외는 ±0% |

## 무엇을 내놓는가

![predict 실행 화면](docs/img/predict.svg)

**Azir 이 이 도구가 하려는 일을 보여 준다.** 승률이 173종 중 172위라 지표만 보면
명백한 버프 대상인데, 프로 경기에 자주 나오는 챔피언이라 버프하면 대회에서의 비중이
크게 오른다(측정값 **+37%**). **순위만으로는 이 판단을 할 수 없다.**
...
시간순 분할(`15_13` 앞뒤) 백테스트다. 무작위 분할은 쓰지 않는다 — 그렇게 자르면 미래 정보가 과거로 누출된다.
...
**LLM 이 통계 베이스라인을 이겼는지도 측정했고, 이기지 못했다.** 대화로 뽑은 규칙
(AUC 0.837)은 로지스틱 회귀와 사실상 동률이고, 검색 위에 올린 판단(0.830)은
다수결(0.850)을 넘지 못했다.
...
판단한 뒤 맞춰 보라는 것**이다. 검색 경계는 `as_of` 로 도구가 지키므로 물어본 패치
...
스캔해도 0.06 ms 라 도입할 이득이 없다. 밀집 임베딩도 측정했으나 희소를 넘지 못했다.
