...
| | |
| --- | --- |
| 데이터 | 52패치 8,767행. 솔랭 승률·픽률·밴율과 프로 경기 픽·밴 29,977경기 |
| 대상 예측 | 173종을 너프 5종·버프 5종으로 좁힌다. 무작위 선택의 **6.2배** |
| 방향 예측 | 너프인가 버프인가. AUC **0.900** (조정된 챔피언 239건) |
| 위험 경고 | 프로 경기 단골을 버프하면 대회 출전이 **+35%**, 그 외는 ±0% |
...
보면 명백한 버프 대상이지만, 프로 경기에 자주 나오는 챔피언이라 버프할 경우
대회에서의 비중이 크게 오른다. 순위만으로는 이 판단을 할 수 없다.

그 경고의 근거는 버프 299건을 두 무리로 갈라 잰 것이다.
...
./scripts/check-all                 # 테스트 340개 · 검사 10개
```
...
스캔해도 0.06 ms 이므로 도입할 이득이 없다. 밀집 임베딩도 측정했으나 희소를 넘지
못했다.
...
검색(RAG)까지 만들고 마무리했다. 조회에서 검색을 거쳐 판단으로 도는 에이전트는 다음
