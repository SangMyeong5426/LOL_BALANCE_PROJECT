...
통계·규칙·검색(RAG)·검색 위의 판단을 **같은 분할과 같은 표본에서 겨루게 해** 어느
갈래가 이기는지 측정한 것이 이 저장소의 답이다.
...
| 위험 경고 | 프로 경기에 자주 나오는 챔피언을 버프하면 대회 출전이 **+35%**, 그 외는 ±0% |
...
| | 무엇을 하나 | 어떻게 됐나 |
| --- | --- | --- |
| 검색부 | 사례 검색(수치 k-NN) · 노트 검색(BM25) · 수치 조회 | 노트 검색 MRR@10 **0.924** · Recall@10 **98.1%** |
| 생성부 | 검색 결과를 읽고 0~100 점과 근거를 매긴다 | AUC 0.830 |

검색부를 쓴 arm 이 전체에서 가장 좋다(AUC **0.900**, 통계 베이스라인 0.796).
반면 생성부는 같은 이웃을 본 다수결 0.850 을 넘지 못했고, 넘지 못했다고 적는다.
