...
| ③ 효과, 의도대로 갔나 | 너프 79.5% · 버프 73.4% | — | 둘이 안 갈린다 (p=0.12) |
| ③′ 효과, 균형에 가까워졌나 | **버프 63.1%** · 너프 53.3% | 50.5% | 버프만 선다 (p<0.0001) |

![기준선 대비 배수](docs/img/lift.svg)

대괄호는 평가 패치 20개를 부트스트랩한 95% 구간이다(`run-metrics`). **폭이 넓다** —
①′ 는 14%p 다. 씨앗은 이 폭에 안 들어간다. 부스팅이 이 설정에서 결정적이라
씨앗을 바꿔도 값이 한 자리도 안 움직인다.
