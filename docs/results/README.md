...
| --- | ---: | ---: | ---: |
| **너프된 챔피언** | 270 | **0.250** | **0.659** |
| **버프된 챔피언** | 400 | **0.079** | **0.473** |
| mixed·adjust | 858 | 0.144 | 0.542 |
| 조정된 것 전부 | 1,565 | 0.145 | **0.543** |
| 조정 안 됨 | 7,037 | 0.110 | |

**너프 쪽은 0.659, 버프 쪽은 0.473.** 합치면 0.543 으로 뭉개진다.
...
| `A7buff` | 버프되나 | 13.5% | 3.5% | **3.8** | 0.708 |
| `A7buffp` | 버프되나 + 프로 | 13.4% | 3.5% | 3.8 | **0.725** |

**R-정확도를 나란히 읽으면 안 된다.** 좁힌 arm 은 패치당 양성이 적어(516 → 113)
기준선이 3%대다. **배수로 본다.**

**「조정되나」로 물으면 1.8배인데 「너프되나」로 좁히면 5.7배다.** 「조정 전부」에는
mixed·adjust 286건이 섞여 있고 그것이 거의 예측이 안 된다.

**프로 피처는 버프 대상에서만 산다**(AUC 0.708 → **0.725**). 너프 대상에서는
...
조정된 챔피언만 본다. **평가 235건**(너프 114 · 버프 121) · **다수 클래스 기준선 51.5%.**
...
| B0c | 밴율 높은 순 | 0.739 | — |
| B0d | 픽률 높은 순 | 0.750 | — |
| B3 | 유사 사례 25개 (k-NN) | 0.763 ↑0.708 | 69.8% |
| B0b | 승률 높은 순 | 0.799 ↓0.836 | — |
| **B6** | **사례 검색 + 판단 — 대화 중** (234/235종) | **0.834** | **77.8%** |
| B1 | 로지스틱 회귀 — 수준 | 0.834 ↑0.759 | 76.2% |
| B2 | 로지스틱 회귀 — 수준 + 추세 | 0.836 ↑0.778 | 76.6% |
| B2h | 로지스틱 회귀 — + 이력·역할 | 0.836 ↑0.785 | 77.4% |
| **B4** | **규칙 엔진 — 대화 중 제안** | **0.838** ↑0.807 | 77.9% |
| B7 | 부스팅 트리 — 수준 + 추세 | 0.856 ↑0.837 | 76.6% |
| B7h | 부스팅 트리 — + 이력·역할 | 0.856 ↑0.832 | 77.0% |
| B5b | 사례 검색만 — 매 패치 갱신 | 0.859 | **78.3%** |
| **B5** | **사례 검색만 (RAG 검색부)** | **0.865** | 75.7% |
| B5s | 사례 검색만 — `B6` 과 같은 234종 | 0.865 | 75.6% |
...
| | | |
| --- | --- | --- |
| `B4` 규칙 | 0.838 | 로지스틱 회귀(`B2h` 0.836)와 **동률** |
| `B6` 판단 | 0.834 | 같은 234종의 다수결(`B5s` 0.865)에 **진다** |

이것이 [CLAUDE.md](../../CLAUDE.md) 가 요구하는 비교다 — **통계 베이스라인 없이
LLM 성능을 주장하지 않는다.** 이기지는 못했고, 못 이겼다고 적는다.

**최고는 사례 검색(B5 0.865)이다.** 승률 한 줄(B0b 0.799)보다 +0.066, 다수
클래스로 찍기(0.515)보다 훨씬 위다.

### 검색 위의 판단(B6)이 왜 다수결을 못 이기나

`B5` 와 **같은 이웃 25종**을 놓고 대화 안에서 0~100 점을 매기고 근거를 적었다.
평가 235건 중 **234건**이다 — `Nunu & Willump` 이름을 고치자 한 종이 늘었고
그 행에는 판단이 없다. 표본이 다르면 짝지어 비교가 안 되므로 `B5` 도 같은
234종으로 잘라 `B5s` 로 함께 실었다.

```
B5  같은 이웃 다수결   AUC 0.865   정확도 75.6%   점수 25종
B6  같은 이웃 + 판단   AUC 0.834   정확도 77.8%   점수 50종
```

**차이 −0.031, 95% [−0.057, −0.005].** 짝지어 부트스트랩 2만 회에서 0 을 안
넘고, `B6` 이 더 높을 확률은 1.1% 다.

**해상도는 병목이 아니었다.** 점수 종류가 25 → 50 으로 두 배가 됐는데 AUC 는
내려갔다. `B4` 에서 해상도를 고쳐 0.807 → 0.838 이 된 것과 다르다.
...
| --- | ---: | ---: | ---: | ---: |
| ◆ B2p 수준+추세+프로 | — | — | 0.860 | **0.881** |
| ◆ B1p 수준 + 프로 | 0.856 | 0.863 | 0.877 | 0.880 |
| **B5 사례 검색** | 0.854 | 0.851 | 0.857 | **0.865** |
| B7 부스팅 | 0.828 | 0.838 | 0.836 | 0.856 |
| B1 회귀 — 수준 | 0.813 | 0.814 | 0.826 | 0.834 |
| B4 규칙 | 0.811 | 0.800 | 0.798 | 0.838 |

마지막 열은 **2026-08-27 재측정**이다(핫픽스 절·이름 어긋남 수정 뒤). `B4` 만
앞 열들과 결이 다른데, 그 사이에 **점수 해상도를 고친 것**이 겹쳐서다 —
0.807 → 0.838 의 경위는 아래 「규칙은 0.807 이었다」에 있다.
...
프로+ · k=25 · 다수결   0.902   프로 피처만으로 거의 전부
```
...
구간에서는 0.860 으로 본 분석 `B5`(0.865)를 못 넘는다.
...
| --- | ---: | ---: | ---: | ---: |
| 너프 | 221 | −1.137%p | **84.6%** | 54.8% |
| 버프 | 346 | +0.961%p | **75.1%** | **64.5%** |
| 조정 안 함 (대조군) | 5,746 | — | — | 50.5% |

**두 열이 다른 말을 한다.** 「의도대로」로 보면 너프가 낫고(84.6% > 75.1%),
「50% 에 가까워짐」으로 보면 버프가 3배 낫다(대조군 대비 **+14.0%p vs +4.3%p**).
...
| 단계 | 질문 | 성적 | 판정 |
| --- | --- | --- | --- |
| 1 · 대상 | 누가 | 기준선의 **1.8배** (너프로 좁히면 **5.7배**) | **약하다.** 좁히면 나아진다 |
| 2 · 방향 | 너프냐 버프냐 | AUC 0.865 (기준선 0.799) | **된다** |
| 3 · 항목 | 무엇을 | — | **아직 안 함** |
| 4 · 폭 | 얼마나 | — | **아직 안 함** |
