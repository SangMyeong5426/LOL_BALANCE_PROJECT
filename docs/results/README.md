...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 부스팅 | **0.893** | **0.895** | 80.7% | 77.5% | 84.5% | 0.809 | 0.617 | 0.134 | 0.417 |
| 회귀 | 0.890 | 0.882 | **83.3%** | 81.0% | **85.5%** | **0.832** | **0.668** | 0.135 | 0.430 |

    혼동행렬(부스팅)   실제 너프 93✓/17✗   실제 버프 91✓/27✗   균형정확도 0.808
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| ① 조정 | 0.634 | 0.277 | 0.272 | 0.146 | 0.573 | 14.7% | 1.9배 |
| **①′ 너프** | **0.789** | 0.258 | 0.236 | **0.209** | **0.604** | 3.4% | **6.9배** |
| ①″ 버프 | 0.734 | 0.199 | 0.164 | 0.133 | 0.567 | 3.6% | 4.6배 |
...
| | R-정확도 |
| --- | ---: |
| 정직하게 (학습 구간으로 학습) | **27.4%** |
| 부정행위 (평가 구간으로 학습) | **47.1%** ← 상한 |
| 완전한 정보 (라벨을 그대로) | 100% |
...
**「0.72」는 72% 가 아니라 32% 쯤이다.** 전체 평균은 14.7% 다.
...
| --- | ---: | ---: | ---: | ---: |
| 조정 후보 | 38.6% | 40.0% | 34.2% | 14.7% |
| → 배수 | 2.6배 | 2.7배 | 2.3배 | |
| **너프 후보** | **31.6%** | 24.2% | 15.8% | **3.4%** |
| → 배수 | **9.4배** | **7.2배** | 4.7배 | |
| 버프 후보 | 17.5% | 14.7% | 12.1% | 3.6% |
| → 배수 | 4.9배 | 4.1배 | 3.3배 | |

**조정 후보가 정밀도는 제일 높은데 배수는 제일 낮다.** 기준선이 14.7% 라 아무나
...
##### 컷은 5 로 뒀다
    ...
    패치당 실제 조정 수   중앙값 21 · 범위 11~41
      ...
      상위    정밀도   맞는 수   재현율   헛짚는 수
        3    38.6%     1.2     5.0%      1.8
        5    40.0%     2.0     8.3%      3.0    ← 정밀도 최고
       10    34.2%     3.4    15.0%      6.6
       20    28.2%     5.6    23.3%     14.4
       40    24.2%     9.7    39.7%     30.3
      (조정 후보 기준. 방향별 값은 위 표)
...
#### 모델 쪽을 일곱 번 밀어 봤다 (2026-08-29 · 08-31) — **전부 실패**
