...
| 방향 | 무엇이 열리나 | 막힌 것 |
| --- | --- | --- |
| 다른 모집단(Elite · Average) | 개발사가 보는 판정 기준 넷 중 둘을 채운다 | **우리 구간에 5패치 이하** ([data-sources](../spec/data-sources.md#우리-구간의-티어-2026-09-01-실측)) |
...
R2 는 검색 성적이 제일 좋은데(MRR@10 0.924 · Recall@10 98.1%) **점수를 내는 arm 이
...
#### 자료 쪽을 밀어 봤다 — **아홉 갈래 · 93회 측정, 전부 실패** (2026-09-01)
...
**절차를 하나로 고정했다.** 갈래마다 다르게 재면 비교가 안 된다.
    ...
    ① 학습 구간(13_14 ~ 15_12)에서 패치 하나씩 앞으로 밀며 20겹
    ② 짝지은 차이의 평균 ± 표준오차. 2 표준오차를 넘어야 「걸렸다」
    ③ 걸린 것만 평가 구간을 한 번 본다

기준 arm 은 `A7p` 다. 값은 ① 0.3412 · ①′ 0.2656 · ② 0.9118 이다.
...
93회 중 2 표준오차를 넘은 것이 **2건**이다. 2 표준오차 기준이면 **우연히 넘을
기대치가 4.6건**이다. 걸린 수가 기대치보다 적다.

##### 잡음 열을 넣어 봤다 — **①′ 의 양수는 전부 이것이다**

위 표는 갈래마다 **열을 전부 넣었을 때**다. 실제로는 갈래를 소집단 31개로 쪼개
3과제씩 93회를 쟀고, **「전부」를 뺀 소집단 22개 중 20개에서 ①′ 너프가 양수다.** 「신호는 있는데 표본이
모자란다」로 읽을 수도 있어, 아무 뜻 없는 난수 열을 같은 수만큼 넣어 봤다.

    난수 열 2개    ① +0.0011   ①′ +0.0121   ② −0.0040
    난수 열 6개    ① −0.0091   ①′ +0.0133   ② −0.0013
    난수 열 12개   ① −0.0060   ①′ +0.0286   ② +0.0094
    난수 열 20개   ① −0.0222   ①′ +0.0061   ② −0.0073
                  (씨앗 5개 평균)

**난수만 넣어도 ①′ 가 +0.012 ~ +0.029 오른다.** 위 표의 ①′ 양수는 전부 이
크기 안이다. 걸린 것으로 셌던 프로 승률·성적의 +0.048 도 여기 묻힌다.

**①′ 는 열을 더하기만 해도 오른다.** 패치당 실제 너프가 3~10종이라 R-정확도가
한 종에 0.1~0.3씩 튀고, 열이 늘면 트리가 그 잡음에 붙을 자리가 생긴다.
반대로 ① 은 열을 더하면 내려간다(난수 20열에 −0.022). **표의 ① 음수도 신호가
나쁘다는 뜻이 아니라 열을 더한 대가다.**

대조군을 안 뒀으면 「프로 승률이 너프 예측을 4.8%p 올린다」로 적었을 것이다.

##### 남은 하나 — 직전 패치의 버프 비율

난수 바닥 위에 남은 것은 하나다. **직전 패치에 이뤄진 조정 중 버프가 몇 %였나**
(`buff_ratio_1`)를 ② 방향에 넣으면 +0.0174 ±0.0061 이다. 20겹 중 11겹에서
오르고 제일 큰 겹을 빼도 +0.0141 ±0.0037 로 남는다.

**패치 안에서 상수인 값이 어떻게 줄 세우기를 바꾸나.** 트리가 그 값으로 갈라
「직전이 버프 편중이었을 때」와 아닐 때 승률을 다르게 읽는다. 상수 이동이 아니라
조건 분기다.

**그런데 저장소 설정으로 재면 무너진다.** 거르는 장치는 `class_weight="balanced"`
를 뺐는데, 저장소의 `_boosting()` 을 그대로 쓰면 +0.0139 ±0.0075 로 **1.9 표준
오차**가 되고 오른 겹도 11에서 6으로 준다.
...
셋 다 오르는 쪽이다. **그래도 채택하지 않는다** — 학습 구간에서 2 표준오차를
못 넘은 것을 평가 구간이 올랐다고 넣으면, 그것이 바로 평가를 보고 고르는 것이다.
[셋째 실패](#모델-쪽을-아홉-번-밀어-봤다-2026-08-29--08-31--전부-실패)에서 같은 자리에 한 번
속았다.
...
| 모집단 | 세다 | 약하다 |
| --- | --- | --- |
| Average (상위 10% 밖) | 승률 54.5%(밴율 평균 이하) ~ 52.5%(평균의 5배) | 승률 49% |
| **Skilled (상위 10%)** | 승률 54% ~ 52%. **2020년에 양쪽 0.5%p 조임** | 승률 49% |
| Elite (상위 0.5%) | **밴율 45%** — 승률은 표본이 얇아 안 쓴다 | 존재감 5% |
| 프로 (상위 5리그) | 존재감 90%, 또는 연속 패치 80% | 존재감 5% |

판정은 **「어느 한 모집단에서라도 상단을 넘으면 세다」**이고, 약한 쪽은
**「모든 모집단에서 하단 아래일 때」**다. 그리고 2020년에 안전장치가 붙었다 —
「연이은 패치의 조정 중 절반 넘게 버프면, 프로 자리별 존재감 1위와 Skilled
자리별 성적(승률+밴율) 1위도 너프를 검토한다」.

**문턱은 실제로 가른다.** 학습 구간 5,334행에서 Skilled 상단을 넘은 행의 조정률이
전체의 두 배다.

    Skilled 상단을 넘음      105행   조정률 42.9%   (안 넘은 5,229행 19.8%)
    어느 모집단에서든 넘음     147행   조정률 44.2%   (5,187행 19.5%)
    자리별 성적 1위          160행   조정률 45.6%   (5,174행 19.4%)
    하단 아래               1,183행   조정률 26.6%   — 그 조정의 95%가 버프

**그런데 모델에 넣으면 안 오른다**(위 표의 마지막 줄). 문턱이 승률과 밴율로 만든
값이고 모델이 그 둘을 이미 본다. **부스팅이 문턱을 스스로 찾는다** — 공개된
숫자를 알려 줘도 새로 아는 것이 없다.

**문턱보다 중요한 것이 그 옆에 있었다.** 개발사는 **모집단 넷**을 따로 보고
「어느 하나에서라도 넘으면 세다」로 판정하는데 우리는 하나 반을 본다 —
[밝혀 두는 한계](#모집단-넷-중-하나-반만-본다)에 적었다.

**버프 편중 안전장치는 우리 구간에서 한 번도 안 켜진다.** 「조정 중 절반 넘게
버프면」이 조건인데 학습 구간의 최근 3패치 버프 비율은 **중앙값 27.3% · 최대
47.3%** 다. 2023~2026 패치는 일관되게 너프 편중이라 **개발사 자신의 안전장치가
발동할 자리가 없었다.**

##### 기준을 잘못 잡을 뻔했다 — **`A7h` 로 재면 안 나올 것이 나온다**

처음에는 스킨·출시 시점을 `A7h`(이력·역할)에 붙여 쟀고 ① +0.022 · ② +0.036 이
나왔다. 둘 다 2 표준오차를 넘어 「걸렸다」로 볼 뻔했다.

**최고 arm 은 `A7h` 가 아니라 `A7p`(프로 경기) 다.** 기준을 옮기니 전부 1 표준오차
안으로 들어갔다. 스킨 줄 수는 픽률과 상관 +0.26 인 인기 대리 지표인데, **프로
픽·밴율이 이미 같은 것을 더 직접 재고 있다.**

**약한 기준에 붙이면 겹치는 신호가 이득처럼 보인다.** 새 피처는 그 과제의 최고
arm 에 붙여야 한다.

## ② 방향 — 너프냐 버프냐

② 를 올릴 후보를 자료 쪽에서 아홉 갈래 찾아봤고 하나만 남았다 —
...
| | |
| --- | --- |
| **모집단 넷 중 하나 반만 본다** | 개발사는 Average · Skilled · Elite · 프로를 따로 보고 「어느 하나에서라도 넘으면 세다」로 판정한다. 우리 자료는 Skilled 하나와 프로뿐이다 — 아래 참조 |
...
「어느 한 모집단에서라도 넘으면 세다」가 공식 기준이므로, **우리가 못 보는 두
모집단에서 넘어 조정된 챔피언은 우리 입력에 근거가 아예 없다.** Elite 는
승률조차 안 쓰고 밴율 45% 로 판정하는데, `emerald_plus` 에서 밴율 45% 를 넘는
행은 학습 구간에 없다.

**아카이브에 다른 모집단이 있기는 하다.** 다만 우리 74패치 구간에는
`emerald_plus` 53패치를 빼면 **전부 5패치 이하**다 — Elite 에 대응하는
`master_plus` 가 5패치, `diamond_plus` 가 3패치다
([티어 실측](../spec/data-sources.md#우리-구간의-티어-2026-09-01-실측)).

**5패치로는 시계열이 안 된다.** [중국 솔랭](../spec/data-sources.md#중국-솔랭은-제외한다--그리고-결과에-명시한다)과 같은 종류의 한계이고, 같은 방식으로 다룬다 — 못 채우므로 적어서 낸다.

이것이 [천장 47.1%](#천장을-쟀다-2026-08-28--274-에서-471-사이가-남은-전부다)의
설명 하나다. 남은 간격이 전부 잡음인 것이 아니라, **개발사가 보는 근거의 일부가
우리 입력에 없다.**
...
**예측 피처로도 대 봤고 안 올랐다** — [자료 쪽을 밀어 봤다](#자료-쪽을-밀어-봤다--아홉-갈래--93회-측정-전부-실패-2026-09-01)의
