...
현재 패치(`16_18`)로 다음 패치를 예측했다. **정답이 아직 없다** — 패치가 나오면
`./scripts/run-live-predict 16_18 --score` 로 채점된다.
...
| 그 패치에 **직접 조정된** 챔피언이 섞여 있었다 — Smolder(−10.2%p) 하나가 무리 평균의 1/3 | 4/20 · 28/110 |
...
수집의 다른 한계도 여기 적는다. 패치당 3,000판은 **선수 257~314명**에서 왔고(선수당
최대 20판), `16_18` 은 출시일을 빼고 8일치뿐이다.
