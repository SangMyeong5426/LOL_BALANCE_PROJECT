...
발견 자체가 없어지는 것은 아니고 — 학습 구간 20겹에서는 ② 방향이 t=3.15 로 확실하다 —
**평가 구간 한 번으로는 ① 에서 갈렸다고 말할 수 없다**가 맞는 표현이다.
...
달랐다.**
    ...
    맥락팩   `make-rag-context` 가 패널 전체를 넘긴다      8,767행
    `B5`    `direction_arms` 가 방향 붙은 행만 쓴다        710행

`k=25` 로 같아도 뽑히는 것이 다르다. 실측하면 이렇다.

    맥락팩 이웃 25종 중 방향이 붙은 것   중앙값 2 · 평균 2.4 · 아예 0인 대상 41/239
...
**공정하게 다시 재려면 판단을 새로 만들어야 한다.** 맥락팩이 `direction_rows` 를
...
너프 자주 나오는 챔피언        0.522      −0.0777    −14.6%   ← 너프 쪽은 차이가 없다
너프 그 외                  0.038      −0.0047    −19.4%
```

**전부 중앙값이다.** 여기 있던 옛 표는 조정 전과 절대 변화를 평균으로, 상대
변화를 중앙값으로 적어 **한 행에 두 통계가 섞여 있었다**(조정 전 0.190 은 평균 ·
중앙값 0.127). 버프 두 줄은 `run-effect` 가 내는 값이고, **너프 두 줄은
[followups 19](../followups.md) 로 열어 둔 자리다** — 스크립트 값과 적혀 있던
값이 안 맞고 어느 쪽인지 못 정했다.
...
| --- | ---: | ---: | ---: | ---: |
| 너프 | 229 | −0.945%p | **79.5%** [73.7, 84.5] | 53.3% [46.6, 59.9] |
| 버프 | 350 | +0.849%p | **73.4%** [68.5, 78.0] | **63.1%** [57.9, 68.2] |
| 조정 안 함 (대조군) | 5,746 | — | — | 50.5% |

**두 열이 다른 말을 한다.** 다만 어느 쪽이 무엇을 말하는지는 구간까지 봐야 갈린다
(2026-09-01 에 붙였다).

    50% 에 가까워짐   버프 63.1% vs 대조군 50.5%   카이제곱 p < 0.0001   ← 확실하다
                    너프 53.3% vs 대조군 50.5%   카이제곱 p = 0.4411   ← 구분 안 된다
    의도대로         너프 79.5% vs 버프 73.4%     카이제곱 p = 0.1182   ← 구분 안 된다

**버프만 균형 쪽으로 확실히 움직인다.** 너프는 대조군과 구분되지 않는다 —
너프해서 승률이 내려가도 이미 5할 아래였으면 균형에서는 멀어지기 때문이다.

한때 여기 「의도대로로 보면 너프가 낫고, 균형으로 보면 버프가 4배 낫다」로
적어 뒀다. **둘 다 검정을 안 붙인 말이었다** — 앞엣것은 p=0.12 라 구분되지 않고,
