...
① 대상  누가 조정되나    R-정확도 27.9% (기준선 14.5% · 1.9배)  ← 잘 안 잡힌다
        누가 너프되나    R-정확도 19.0% (기준선  3.3% · 5.7배)  ← 좁히면 낫다
...
| **A7p** | **부스팅 트리 — + 프로 경기** | **27.9%** | **32.0%** | **0.617** |
| A7h | 부스팅 트리 — + 이력·역할 | 25.6% | 31.5% | 0.611 |

**최고가 기준선의 1.9배다.** 절대값이 27.9% 라는 것은 **넷 중 셋은 못 맞힌다**는 뜻이다.
...
| mixed·adjust | 855 | 0.144 | 0.544 |
| 조정된 것 전부 | 1,521 | 0.146 | **0.546** |
| 조정 안 됨 | 7,081 | 0.110 | |

**너프 쪽은 0.658, 버프 쪽은 0.473.** 합치면 0.546 으로 뭉개진다.
...
| --- | --- | ---: | ---: | ---: | ---: |
| `A7p` | 조정되나 | 27.9% | 14.5% | **1.9** | 0.617 |
| `A7nerf` | 너프되나 | 19.0% | 3.3% | **5.7** | 0.772 |
| `A7nerfp` | 너프되나 + 프로 | 18.9% | 3.3% | 5.7 | **0.779** |
| `A7buff` | 버프되나 | 12.0% | 3.5% | **3.4** | 0.703 |
| `A7buffp` | 버프되나 + 프로 | 12.3% | 3.5% | 3.5 | **0.727** |

**R-정확도를 나란히 읽으면 안 된다.** 좁힌 arm 은 패치당 양성이 적어(497 → 114)
기준선이 3%대다. **배수로 본다.**

**「조정되나」로 물으면 1.9배인데 「너프되나」로 좁히면 5.7배다.** 「조정 전부」에는
mixed·adjust 269건이 섞여 있고 그것이 거의 예측이 안 된다.

**프로 피처는 버프 대상에서만 산다**(AUC 0.703 → **0.727**). 너프 대상에서는
승률·밴율이 같은 정보를 이미 담아 중복이다(0.772 → 0.779).
...
| | | |
| --- | --- | --- |
| `B4` 규칙 | 0.836 | 로지스틱 회귀(`B2h` 0.836)와 **동률** |
| `B6` 판단 | 0.834 | 다수결(`B5` 0.866)에 **진다** |
...
평가 234건 전량이다 — 표본이 다르면 짝지어 비교가 안 된다.
...
방향 라벨 1,555종을 대화 안의 Claude 가 붙였고 그것이 곧 채점 정답이다.
...
**4.6배다.** 이 구간에서는 프로 픽·밴율이 승률보다 잘 가른다(AUC 0.771 vs 0.675).
...
**비율로는 프로 단골이 덜 빠진다. 들어오기는 쉽고 나가기는 어렵다.**

**평균회귀로는 설명이 안 된다.** 프로 단골은 프로 픽·밴율이 튄 패치에 너프당하므로 자기
...
너프 프로 단골        0.551      −0.1062    −14.6%   ← 너프 쪽은 주장을 철회한다
...
구간에서는 0.860 으로 본 분석 `B5`(0.866)를 못 넘는다.
...
| 단계 | 질문 | 성적 | 판정 |
| --- | --- | --- | --- |
| 1 · 대상 | 누가 | 기준선의 **1.9배** (너프로 좁히면 **5.7배**) | **약하다.** 좁히면 나아진다 |
...
| **에이전트가 없다** | API 키가 필요하다. **단발 판단은 `B6` 으로 쟀고**(다수결에 진다) 남은 것은 조회→검색→판단 루프다 |
...
판단은 다수결에 진다. **남은 것은 「단발 판단」이 아니라 「루프」다.**
