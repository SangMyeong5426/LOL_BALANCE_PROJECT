...
**패치 단위 leave-one-out** 이라 같은 패치가 학습과 평가에 같이 들어가지 않는다.
    ...
    판단 점수 재구성            R² 0.867 · 상관 0.932
    AUC   원본 판단 0.8301      재구성 0.8630     ← 재구성이 더 낫다
    판단 − 재구성(잔차)만으로     AUC 0.5081       ← 얹은 것에 방향 신호가 없다

**판단자가 선형 읽기 위에 얹은 것은 찍기와 구별되지 않고, 그 얹음이 AUC 를
0.03 깎는다.** 이웃(중앙값 2종)도 노트도 점수를 바꾸지 못했다.

「판단이 검색에 닻을 내린다」로 적어 두었던 설명은 이것으로 바뀐다 — **닻을 내릴
검색이 애초에 거의 없었고, 판단자가 실제로 읽은 것은 대상의 수치였다.**
