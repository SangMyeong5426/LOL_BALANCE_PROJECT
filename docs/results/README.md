...
**견준 것**: 패치마다 u.gg 스냅샷 전 4,500판(`kr` · `euw1` · `na1` 각 1,500판). `16_15` 는
그 뒤로 3,000판을 더 모았지만 스냅샷 뒤 경기라 여기서는 안 센다. 지금 에메랄드
이상인 선수를 단계별 인원에 비례해 뽑아 그 경기를 받았다. 두 패치 모두 u.gg 의
스냅샷 시각 전 경기만 센다.

| 패치 | 직접 집계 | u.gg | 창 |
| --- | ---: | ---: | --- |
| `16_15` | 4,500판 · 선수 806명 (모은 것은 7,500판 · 1,081명) | 334,500판 | 출시 ~ 07-31 14:05Z |
| `16_13` | 4,500판 · 선수 397명 | 2,254,893판 | 출시 ~ 07-06 18:34Z |

### 승률은 구분되지 않고, 픽률·밴율은 계통 차이가 남는다

| 패치 | 비율 | r | 평균 차이 | z 평균 | z 표준편차 | 계통 차이 | 판정 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| `16_15` | 승률 | 0.401 | +0.01%p | −0.00 | 1.02 | 0.99%p | **잡음 안** |
| `16_15` | 픽률 | 0.984 | +0.00%p | −0.03 | 1.50 | 0.50%p | 잡음 밖 |
| `16_15` | 밴율 | 0.990 | +0.08%p | +0.08 | 1.93 | 0.92%p | 잡음 밖 |
| `16_13` | 승률 | 0.304 | −0.15%p | −0.02 | 1.06 | 1.50%p | **잡음 안** |
| `16_13` | 픽률 | 0.983 | +0.00%p | −0.10 | 1.49 | 0.51%p | 잡음 밖 |
| `16_13` | 밴율 | 0.984 | +0.09%p | −0.08 | 2.26 | 1.28%p | 잡음 밖 |
...
승률 폭이 그만큼이라 상관이 희석된다. z 로 보면 평균 −0.02~−0.00 · 표준편차
1.02~1.06 으로, **잡음으로 설명되지 않는 차이가 안 보인다.** 다만 이것은 「같다」가
...
```
우리 · 밴 칸 그대로        9.635개/판
우리 · 같은 챔피언 한 번    9.084종/판     ← 경기의 45.2% 에서 중복 밴이 있다
u.gg                      8.939/판
```

u.gg 는 한 경기에서 두 팀이 같은 챔피언을 밴해도 **한 번으로 센다.** 정의를 맞추니
`16_15` 밴율이 z 평균 +0.42 → **+0.08**, 계통 차이 1.49%p → **0.92%p** 로 줄었다.
...
(`16_17` 19종 · `16_18` 12종 · 기계 판정과 일치 10 · 확장 2 · 충돌 0 — 처음 붙인 넷이 라벨 기준에 어긋나 감사 뒤 고쳤다, [ADR 0012](../adr/0012-predicting-with-direct-aggregation.md)).

| 기준 → 정답 | 대상 종수 | 그 패치 조정률 | 너프 상위 5 중 조정 | 그중 방향까지 |
| --- | ---: | ---: | ---: | ---: |
| `16_16` → `16_17` | 51종 | 19.6% | **1종** (기준선 1.0) | 1종 (Nasus) |
| `16_17` → `16_18` | 51종 | 11.8% | 0종 (기준선 0.6) | 0종 |

**이 숫자로는 아무 말도 못 한다.** 평가 지점이 둘이고, 상위 5 안의 적중이 1종과
0종이다. 기준선과 같다.

**이 표는 표본에 흔들린다.** `16_15` 를 4,500판에서 7,500판으로 늘리자 `16_16` 행의
추세 피처가 움직여 너프 상위 5 가 바뀌었다(Yasuo 가 빠지고 Lee Sin 이 들어와 **2종 →
1종**). 채점 하나가 챔피언 하나에 달린 크기다.
...
| 걸린 것 | 크기 |
| --- | --- |
| **`16_15` 표본이 처음 2.5일에 60% 몰려 있다** (u.gg 비교 창을 먼저 채웠다). 아이템 변경이 없는 **같은 패치 안**에서 앞 반 → 뒤 반을 견주니 차이가 **−1.30%p** 로 첫 판과 같았다 | 첫 판의 차이 전부 |
...
| --- | --- | ---: | ---: | --- | ---: |
| `16_15` → `16_16` | 아이템이 세진 13종 | +0.06%p | +0.07%p (82종) | −0.01 [−3.06, +3.23] | 3.7%p |
| `16_15` → `16_16` | 아이템이 약해진 3종 | −6.73%p | +0.07%p | −6.80 [−13.86, −0.16] | 8.4%p |
| `16_16` → `16_17` | 아이템이 약해진 9종 | +0.71%p | +0.46%p (95종) | +0.25 [−2.97, +3.42] | 4.8%p |
| `16_17` → `16_18` | 아이템 값 변경 없음 | — | — | — | — |

**아무것도 못 본다.** 세진 13종은 차이가 0 이고, 약해진 3종은 구간이 0 을 살짝
비켜 가지만 **세 종으로 낸 부트스트랩 구간은 믿을 것이 못 된다**(가를 수 있는
최소 차이 8.4%p 가 관측 6.8%p 보다 크다). 아이템 하나가 낼 법한 차이(1%p 아래)는
이 표본이 가를 수 있는 것(3~5%p)보다 훨씬 작다 — **「구분 안 된다」는 가설과 무관하게
나오는 결과다.**
...
| --- | ---: | ---: | ---: |
| `16_16` → `16_17` 조정 | 0/20 | 16/110 | 0.057 |
| `16_17` → `16_18` 조정 | 0/9 | 9/111 | 0.483 |
...
수집의 다른 한계도 여기 적는다. 패치당 3,000판은 **선수 257명**에서 왔고(선수당 최대
20판), `16_18` 은 출시일을 빼고 8일치뿐이다.
...
| **출처 재현** | 직접 모은 4,500판이 u.gg 승률과 구분되지 않는다. 픽률·밴율은 0.5~1.3%p 차이가 남는다 ([위](#직접-모은-경기로-솔랭-지표를-재현해-봤다-2026-09-20)) |
