...
`B5` 는 이웃 `k` 개를 찾아 **라벨을 다수결**한다 — 생성 단계가 없다. 검색기 셋 중
**노트 검색(R2)·수치 조회(R3)는 만들어 두고 아직 아무 arm 도 안 쓴다.**
「에이전트가 쥘 도구」로 만들었고 에이전트가 아직 없어서다.
용어는 [glossary](../glossary.md#rag--검색부만-있다) 참조.
