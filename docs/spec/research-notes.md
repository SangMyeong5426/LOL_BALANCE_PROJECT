...
1차 조사 이후 범위가 크게 바뀌어(3막까지, 카드 풀 전체, AI 드래프트) 다시 조사했다.

## 8. 카드 보상 규칙 — **전부 확보**

출처: [spire-codex.com/mechanics/card-rarity](https://spire-codex.com/mechanics/card-rarity)
...
**승천 7 이상에서 레어가 줄어든다** — 일반 전투 61.5 / 37 / **1.49**, 엘리트 54.9 / 40 / **5**. 승천 0을 택한 결정이 여기에도 영향을 준다(레어가 두 배 자주 나온다).
...
```
시작값        −5%
레어가 안 나오면   +1%  (승천 7 이상은 +0.5%)
상한          +40%
레어가 나오면     −5% 로 초기화
```

**이걸 빼먹으면 레어 카드 전체가 저평가된다.** 일반 전투 기본 확률이 3%뿐이라 보정이 실질 확률을 지배한다.
...
`open-questions.md` 로 넘기지 않고 **단위 테스트로 못 박는다** — 10만 회 뽑아 등급 분포가 위 표와 맞는지, 오프셋이 실제로 누적·초기화되는지, 한 보상에 중복이 없는지. 데미지 계산과 같은 급의 항목이다. 틀려도 에러가 안 나고 결과만 조용히 오염된다.

## 9. 무색 카드 — **1단계 범위에서 빠진다**

출처: [sts2front.com](https://sts2front.com/tips/colorless-cards-guide/)

무색 카드는 **60장 이상**이고 커먼·언커먼·레어에 고루 분포한다. 그런데 획득 경로가 이렇다.

| 경로 | 비고 |
| --- | --- |
| 상점 | 상점마다 2장 판매 (60~180 골드) |
| Unknown room 이벤트 | 특정 이벤트 보상 |
| Dingy Rug 유물 | 전투 후 보상에 무색 1장 추가 |
| Toolbox / Orange Dough 유물 | 전투 시작 시 지급 |

**일반 전투 카드 보상에는 나오지 않는다.**

1단계는 상점·이벤트·유물을 만들지 않으므로 **무색 카드는 들어올 경로가 없다.** 구현 대상에서 빠진다. 상점과 이벤트를 만드는 단계에서 함께 들어온다.

## 10. 적과 보스 — 거의 전부 고정 패턴

### 2막 적 (표본)

**Spiny Toad** — 체력 116~119 (A8 121~124)

| 기술 | 효과 | A9 |
| --- | --- | --- |
| Protruding Spikes | 가시 5 획득 | — |
| Spike Explosion | 23 피해 + 가시 5 감소 | 25 |
| Tongue Lash | 17 피해 | 19 |

**패턴**: 위 셋을 순서대로 고정 순환. 확률 분기 없음.

**1막과 같다.** 2막 적도 결정론적으로 보인다. 다만 표본이 1종이라 더 확인이 필요하다.

### 보스 — 막당 3종씩, 대부분 고정

출처: [sts2front.com](https://www.sts2front.com/tips/all-bosses-act-by-act/)

| 막 | 보스 | 패턴 |
| --- | --- | --- |
| 1 (Forest) | Ceremonial Beast (252~262) | **고정.** 2페이즈 — 1턴 Stamp 로 Plow 150 축적 → 매 턴 Plow(18 피해, 힘+2). 체력 150 이하에서 기절하며 힘 소실 → 이후 Beast Cry(Ringing 1) → Stomp(15) → Crush(17, 힘+3) 3턴 순환 |
| 1 | Kin Squad (사제 190 + 추종자 58~59) | 고정. 사제가 죽으면 추종자도 소멸 |
| 1 | Inkblot Phantasm | 고정. 1턴에 30 피해 + Call 상태 카드 3장. 2턴마다 무형 |
| 1 (Darkport) | Waterfall Beast | 고정. 매 턴 Steam 축적, 죽을 때 자폭 |
| 1 | Soul Fysh | 고정. 무형 반복 + Call 카드 주입 |
| 1 | Sleeping Gargoyle | **변수 있음** — 수면이 2~3턴 |
| 2 (Hive) | Knowledge Demon | 고정. 1턴 대기, 2턴에 상태 카드 강제 선택 |
| 2 | Kaiser Crab (왼발 199 + 오른발 189) | 위상 변화 있음. **방향 기반 피해 보정** — 뒤에서 치면 50% 증가 |
| 2 | Insatiable | 고정. 모래 구덩이 카운트다운, 끝나면 즉시 패배 |
| 3 (Glory) | Queen + Torch Head (400 + 199) | 고정. Queen 이 덱 최상단 3장에 Erode |
| 3 | Test Subject C10 | **3페이즈.** 죽일 때마다 머리 증가, 최종 페이즈 2턴마다 무형 축적 |
| 3 | Doormaker (489~512) | 고정. 2페이즈. Hunger → Scrutiny → Grasp 루프 |

**보스는 일반 적보다 비싸다** — 페이즈 전환, 다중 개체, 방향 판정, 즉사 타이머 같은 특수 규칙이 붙는다. 다만 **확률 분기는 거의 없다.**

**Kaiser Crab 의 방향 판정**은 이 프로젝트에 없는 개념이다(위치/방향을 모델링하지 않음). 이 보스를 넣으려면 규칙을 하나 더 만들어야 한다.

## 11. 검증 데이터 — **핵심 출처를 찾았다**

### gaistou 커뮤니티 런 통계

[gaistou.github.io/sts2-stats](https://gaistou.github.io/sts2-stats/) — **표본 약 512,000 런** (spire-codex 제출분)

**이 사이트가 0-A-1 의 문제를 전부 해결한다.**

| 컬럼 | 내용 | 이 프로젝트에서 |
| --- | --- | --- |
| **N** | 9층까지 픽업 횟수 | **표본 수 공개** — 저표본 카드를 거를 수 있다 |
| **With / Without** | **Act 1 클리어율** (카드 있을 때 / 없을 때) | **계획서 3-1 의 지표가 실측에 그대로 있다** |
| **Diff** | 위 둘의 차이 | 우리 시뮬레이터 값과 직접 대조 |
| **Global winrate** | 3막 전체 승률 | Act 1 성능과 런 성능의 격차 분석 |
| `Δ (A9/A10 only)` | 승천 9~10 구간 별도 | **승천 구간을 구분해 준다** |
| `Last changed patch` | 카드별 마지막 변경 패치 | **패치 이후 데이터만 집계** — 기준선이 이미 정리돼 있다 |
...
| --- | --- |
| 표본 수를 모른다 | `N` 컬럼 |
| Act 1 만 분리할 수 없다 | `With/Without` 이 Act 1 클리어율이다 |
| 승천 구간을 못 맞춘다 | 승천 구간 컬럼이 있다 |

**픽률을 표본 수의 대리 지표로 쓰는 우회는 필요 없어졌다.**

### spire-codex 커뮤니티 통계

[spire-codex.com/community-stats](https://spire-codex.com/community-stats) — **표본 1,323,090 런**, 승리 344,235 (**승률 26%**)

- **층별 생존율** 그래프가 있다 — 성능 곡선(3-x)의 대조 대상 후보. 다만 **그래프라 수치 추출이 어렵다**
- 캐릭터별·승천별 승률, 캠프파이어 선택, 가장 치명적인 적과 이벤트

### 캐릭터별 승률 (MetaBot.GG)

Silent 15.5% (체력 70) · **아이언클래드 14.9% (체력 80)** · Necrobinder 14.8% (66) · Regent 14.3% (75) · Defect 11.2% (75)

**출처마다 전체 승률이 다르다** (26% vs 15%). 승천 구간과 집계 방식이 다를 것이다. 대조에 쓸 때 어느 값을 쓰는지 명시해야 한다.

## 2차 조사 결론

| 항목 | 결과 |
| --- | --- |
| 카드 보상 규칙 | **확보.** 단위 테스트로 못 박는다 |
| 무색 카드 | **1단계에서 빠진다.** 전투 보상에 안 나온다 |
| 2막 적 | 표본 1종이 고정 순환. 1막과 같아 보인다 |
| 보스 | 막당 3종. **거의 전부 고정.** 다만 페이즈·다중 개체·특수 규칙이 붙어 비싸다 |
| 층별 사망 분포 | 그래프로 존재. 수치 추출은 별도 문제 |
| **검증 데이터** | **gaistou 가 Act 1 클리어율 · 표본 수 · 승천 구간 · With/Without 을 전부 준다** |
