...
def as_patch(text: str) -> str:
    ...
    **같은 패치를 출처마다 다르게 부른다** — 통계 사이트 화면은 `16.15`, 이
    ...
    날짜와 내부 ID 로 잇는다(`CLAUDE.md` 「버전 표기를 믿지 않는다」).
    """
...
class PanelRow:
    ...
    #
    # 방향 예측에서 값을 한다(회귀 AUC 0.833 → 0.879).
    #
    # **① 대상에서는 방향에 따라 부호가 반대라 상쇄된다** — 너프 쪽은 「조정 안
    # 됨」과 AUC 0.658 로 갈리는데 버프 쪽은 0.473 이다. 그래서 회귀에 그냥
    # 넣으면 0.596 → 0.597 로 안 움직인다. 부스팅(A7p)이나 방향을 좁힌
    # arm(A7buffp)에서는 살아난다.
