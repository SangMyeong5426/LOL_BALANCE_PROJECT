...
class NoteSearch:
    ...
    **BM25 를 쓴다. 재 보고 골랐다** — 「챔피언 + 패치」로 그 블록을 찾는 과제
    1,599건에서 이렇다.

        BM25 (B=0.5)         MRR@10 0.946 · Recall@10 99.2%   ← 이것
        BM25 (B=0.75)        0.924 · 98.1%
        TF-IDF 희소           0.697 · 93.1%
        TF-IDF + SVD 밀집     0.504 · 79.4%
        qwen3-embedding      0.480 · 82.1%    로컬 트랜스포머 임베딩
        nomic-embed-text     0.366 · 71.1%    로컬 트랜스포머 임베딩

    **밀집 벡터가 희소보다 나쁘다.** 챔피언 이름·패치 번호·변경 수치처럼 정확
    일치가 중요한 코퍼스라 압축이 손해다. 새 의존성도 안 는다.

    **트랜스포머 임베딩도 재 봤다 (2026-09-18) — 더 나쁘다.** 챔피언은 맞히고
    패치를 못 가린다(1위가 같은 챔피언의 다른 패치인 건이 1,047~1,222 / 1,599).
    BM25 와 섞어도(RRF) 0.735 로 내려간다. `scripts/score-retrieval --embed` 로
    다시 잰다.
    """

    K1 = 1.5

    # 길이 정규화의 세기. **기본값 0.75 는 이 코퍼스에 너무 세다.**
    #
    # 블록 길이가 중앙값 20 토큰인데 편차가 크다. 0.75 로 재면 짧은 블록 하나가
    # 두 낱말을 다 맞힌 긴 블록을 이긴다 — 「Heimerdinger 13_15」 질의에서
    # `16_3 Stats`(10토큰, heimerdinger 만 맞음) 7.77 이 `13_15 UPGRADE!!!`
    # (68토큰, 둘 다 맞음) 6.17 을 눌렀다.
    #
    # 그래서 **놓치는 질의의 정답 블록이 유독 길다** — 중앙값 75 토큰으로
    # 전체 중앙값 20 의 네 배다. 1위가 다른 패치의 같은 챔피언으로 나오는 163건도
    # 같은 기전이다.
    #
    # **앞 절반 패치(916건)에서 고르고 뒤 절반(683건)으로 확인했다.**
    #
    #     B      앞 절반 MRR   뒤 절반 MRR   뒤 Recall
    #     0.75      0.9258      0.9206       97.5%
    #     0.5       0.9472      0.9432       99.3%   ← 이것
    #     0.3       0.9450      0.9452       99.6%
    #     0         0.9028      0.9031      100.0%   ← 1위 정확도가 82%로 무너진다
    #
    # 0 으로 두면 재현율은 100% 인데 순서가 나빠진다. 길이를 아예 안 보면 긴
    # 블록이 낱말 수만으로 이긴다.
    B = 0.5
