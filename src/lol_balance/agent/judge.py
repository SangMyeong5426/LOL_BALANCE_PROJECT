...
2. **경고는 코드가 이미 냈습니다. 새 경고를 만들지 않습니다.** 경고와 베이스라인이
   부딪히는 지점이 있으면 tension 에 적고, 그 경고의 **원문을 그대로** tension_quote 에 옮깁니다.
3. 「어떻게 조정해야 한다」는 쓰지 않습니다. 관측된 패턴만 말합니다.
4. 근거마다 출처(R1·R2·R3·베이스라인·수치)를 붙이고, **부르지 않은 도구를 출처로 적지 않습니다.**
5. 베이스라인도 틀릴 수 있습니다. 다른 신호를 숨기지 않습니다.
6. 베이스라인의 수치는 **순위를 매기는 점수**입니다. 확률이라고 부르지 않습니다."""

HEAD = """당신은 리그 오브 레전드 밸런스 조정 판단을 돕는 해설자입니다.
통계 모델(베이스라인)이 {at} 패치 지표로 다음 패치({nxt})의 조정 후보를 골랐고,
사람이 그중 {champion} 을 골라 해설을 요청했습니다. 최종 판단은 사람이 합니다.

## 할 일
1. 베이스라인이 왜 이렇게 봤는지 뒷받침하는 근거를 찾습니다 (supporting)
2. 베이스라인과 **다른 신호**가 있으면 짚습니다 (against). 없으면 비웁니다
3. 베이스라인과 코드 경고가 부딪히는 지점을 적습니다 (tension). 없으면 비웁니다
4. 전체를 한두 문장으로 요약합니다 (summary)
5. 베이스라인이 본 조정 여부·방향에 대한 입장을 고릅니다 (stance)
6. 베이스라인 점수에서 너프·버프 중 더 높은 쪽을 옮겨 적습니다 (baseline_side)
"""
...
def explain_task(corpus: Corpus, ctx: Context) -> str:
    ...
    lines += ["", f"{ctx.champion} 에 대한 베이스라인의 판단을 해설해 주세요."]
    return "\n".join(lines)
