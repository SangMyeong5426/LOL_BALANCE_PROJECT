...
# 뽑는 이웃 수. `B5` 와 같은 25 다.
#
# **`k` 만 같고 검색 풀이 다르다 (2026-09-01 확인).** 의도는 「같은 검색 결과
# 위에서 판단해야 「다수결 대신 판단하면 나아지는가」를 재는 것이 된다」였는데,
# `make-rag-context` 는 패널 전체(8,767행)를 넘기고 `direction_arms` 는 방향
# 붙은 행(710행)만 쓴다. 그래서 판단자가 본 **방향 붙은 이웃은 중앙값 2종**이고
# 41/239 는 아예 0종이다.
#
# **고치지 않는다.** 이 코드가 저장소에 든 판단 314건을 만든 그 코드다. 풀을
# 바꾸면 그 판단들이 무엇을 보고 나온 것인지 재현할 수 없게 된다. 공정한
# 재측정은 판단을 새로 만들어야 하고, 그 조건은 followups 21 에 있다.
CASE_COUNT = 25
